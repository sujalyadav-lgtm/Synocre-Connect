...
import { getProducts, createOrder } from "@/lib/firebase/db";
...
export default function NewPurchasePage() {
  ...
  const handleSubmit = async () => {
    if (!vendorName || lineItems.length === 0 || lineItems.some(i => !i.productId || i.quantity <= 0)) {
    ...
    try {
      const items = lineItems.map(item => {
        ...
        return {
          ...
          totalPrice: product.costPrice * item.quantity,
        };
      });

      await createOrder({
        type: "purchase_order",
        status: "confirmed", // Confirmed but not yet fulfilled/received
        items,
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Create a PO to restock inventory from a supplier.</p>
      </div>
            ...
            <div key={index} className="flex items-center gap-4 border p-4 rounded-md">
              <div className="flex-1">
                <Select value={item.productId} onValueChange={(val) => updateLineItem(index, "productId", val)}>
                  ...
                  <SelectContent>
                    {products.map(p => <SelectItem key={p.id} value={p.id}>{p.name} - ${p.costPrice} (Cost)</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="w-24">
      ...
      <div className="flex justify-end gap-4">
        ...
        <Button onClick={handleSubmit} disabled={saving || lineItems.length === 0 || !vendorName}>
